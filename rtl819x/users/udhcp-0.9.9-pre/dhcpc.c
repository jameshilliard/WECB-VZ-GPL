...
void sendEventMessage(unsigned char assigned, const char *ip, const char *mask, const char *gateway, const char *nameserver)
{
    ...
    */
    ...
    if( assigned ) {
        ...
        snprintf(connreq_url, sizeof(connreq_url), "http://%s:%d", ip, TR69_PORT);
        ...
        if (b_setconnrequrl) {
            ...
            if (tr69_get_unfresh_leaf_data("Device.ManagementServer.X_ACTIONTEC_COM_UUID", (void *)&p_str, &type)==0) {
                if (type==TR69_NODE_LEAF_TYPE_STRING && p_str) {
                    snprintf(connreq_url, sizeof(connreq_url), "http://%s:%d/%s", ip, TR69_PORT, p_str);
...
{
#if defined(UDHCPC_PASS_DOMAINNAME2DHCPSERVER) || defined(CONFIG_RTL865X_KLD)
...
//#ifdef CONFIG_RTL865X_AC /* In order to accept empty character in HostName. Keith */	
    ...
    // If AEI_CTL_BRIDGE were not defined, 'sysconf' would sleep 10 seconds before launch udhcpc.
    sleep(10);
#endif
#endif
	...
	if (detect) {
		...
		system("echo \"1\" > /var/ulinker_auto_dhcp");
					...
					if (!client_config.foreground)
						background();
#ifdef CHECK_SERVER_ALIVE
					if (client_config.alive_time && t1 > (unsigned long) client_config.alive_time) {
