...
static int flash_read_conf(char *buf, int offset, int len);
...
static int setSystemTime_flash(void)
{
	...
	// CID 25406: Uninitialized scalar variable (UNINIT)
	char buffer[200] = {0};
#else
	char buffer[200];
#endif
...
#ifdef AEI_COVERITY_FIX
	// CID 25405: Uninitialized array index read (UNINIT)
...
#ifdef AEI_COVERITY_FIX
			// CID 25106: Resource leak (RESOURCE_LEAK)
			fclose(fp);
#endif
			return;
		}
...
static int resetDefault(void)
{
	...
	fh = open(FLASH_SAVE_CONFIG, O_RDWR);
...
#endif
#ifdef AEI_COVERITY_FIX
		// CID 25109: Resource leak (RESOURCE_LEAK)
		close(fh);
#endif
		return -1;
	}
...
#ifdef AEI_COVERITY_FIX
	// CID 25140: Dereference before null check (REVERSE_INULL)
	if(pfile == NULL)
	{
		close(fh);
		return -1;
...
#endif		
#if defined(AEI_WECB)
        else if ( !strcmp(argv[argNum], "rescure_wsc_pin") ) {
            return act_rescure_wsc_pin();
        }
        else if ( !strcmp(argv[argNum], "reset_wsc_pin") ) {
            return act_reset_wsc_pin();
        }
#endif
	}
...
#ifdef __mips__
	fh = open(FLASH_SAVE_CONFIG, O_RDWR | O_CREAT | O_TRUNC);
...
#ifdef AEI_COVERITY_FIX
	// CID 29760: Argument cannot be negative (REVERSE_NEGATIVE)
#else
	write(fh, buff, HW_SETTING_SECTOR_LEN + DEFAULT_SETTING_SECTOR_LEN +
		CURRENT_SETTING_SECTOR_LEN);
...
#ifdef AEI_COVERITY_FIX
		// CID 25112: Resource leak (RESOURCE_LEAK)
		free(buff);
#endif
		return -1;
	}
...
#ifdef MIB_TLV
#if 1	
	if ( flash_read_conf(hw_setting_start, HW_SETTING_OFFSET, 6)==0 ) {
...
#endif
#ifdef AEI_COVERITY_FIX
			// CID 25113: Resource leak (RESOURCE_LEAK)
			close(fh);
#endif
			return -1;
		}
...
#ifdef VAP_WPS
				pMib->wlan[idx][i+1].wscMethod = 3;
				pMib->wlan[idx][i+1].wscAuth = WSC_AUTH_OPEN; //open
				pMib->wlan[idx][i+1].wscEnc = WSC_ENCRYPT_NONE; //open
				pMib->wlan[idx][i+1].wscUpnpEnabled = 1;
#endif				
...
//mib_display_data_content(DEFAULT_SETTING, pMib, sizeof(APMIB_T));	
	...
	pfile = malloc(mib_tlv_max_len);
#ifdef AEI_COVERITY_FIX
	// CID 25141: Dereference before null check (REVERSE_INULL)
	if(pfile == NULL)
	{
		close(fh);
		return -1;
...
#endif // #ifdef COMPRESS_MIB_SETTING	
	if ( flash_read_conf((char *)&header, offset, sizeof(header)) == 0) {
		printf("read hs header failed!\n");
		return -1;
	}
	offset += sizeof(header);
	if ( flash_read_conf((char *)buff, offset, header.len) == 0) {
...
static char *webpage_dev_name[2]=
{
...
static int get_actvie_bank()
{
	FILE *fp;
#ifdef AEI_COVERITY_FIX
	// CID 29820: Uninitialized scalar variable (UNINIT)
	char buffer[2] = {0};
#else
	char buffer[2];
#endif
...
static int flash_read(char *buf, int offset, int len)
{
	...
	active_bank = get_actvie_bank();
	fh = open(webpage_dev_name[active_bank - 1], O_RDWR);
#else
...
static int flash_write_conf(char *buf, int offset, int len)
{
	int fh;
	int ok=1;
...
#ifdef AEI_COVERITY_FIX
		// CID 25107: Resource leak (RESOURCE_LEAK)
		if (file)
		{
			close(fh);
		}
...
//printf("web size=%ld\n", header.len);
#ifdef AEI_COVERITY_FIX
	// CID 25154: String not null terminated (STRING_NULL)
	buf = malloc(header.len + 1);
#else
...
#ifdef AEI_COVERITY_FIX
					// CID 25108: Resource leak (RESOURCE_LEAK)
					free(buf);
#endif
					return -1;
				}
...
{
#ifdef AEI_COVERITY_FIX
	// CID 25392: Uninitialized scalar variable (UNINIT)
	// CID 25393: Uninitialized scalar variable (UNINIT)
	// CID 25394: Uninitialized scalar variable (UNINIT)
	// CID 25395: Uninitialized scalar variable (UNINIT)
	int fh = 0, intVal = 0, encrypt = 0, enable1x = 0, wep = 0;
	// CID 25391: Uninitialized scalar variable (UNINIT)
...
#ifdef AEI_COVERITY_FIX
	// CID 29833: Uninitialized scalar variable (UNINIT)
	int wlan_mode = 0;
#else
	int wlan_mode;
#endif
...
static int generateHostapdConf(void)
{
	...
	FILE *fh = NULL;
#ifdef AEI_COVERITY_FIX
	// CID 25386: Uninitialized scalar variable (UNINIT)
	int intValue = 0;
#else
	int intValue;
#endif
...
#ifdef AEI_COVERITY_FIX
	// CID 25387: Uninitialized scalar variable (UNINIT)
	char line[1024] = {0}, strValue[1024] = {0};
#else
	...
	if (!fh) {
		...
		*/
		...
		{
#ifdef AEI_COVERITY_FIX
			// CID 25388: Uninitialized scalar variable (UNINIT)
			int count = 0;
#else
			int count;
#endif
...
#ifdef AEI_COVERITY_FIX
	// CID 29834: Uninitialized scalar variable (UNINIT)
	int intVal = 0;
#else
	int intVal;
#endif
...
{
#ifdef AEI_COVERITY_FIX
	// CID 25407: Uninitialized scalar variable (UNINIT)
	// CID 25408: Uninitialized scalar variable (UNINIT)
	int i = 0, i1 = 0, i2 = 0;
#else
	int i, i1, i2;
#endif
...
static int initWlan(char *ifname)
{
...
#ifdef AEI_COVERITY_FIX
	// CID 25396: Uninitialized scalar variable (UNINIT)
	// CID 25398: Uninitialized scalar variable (UNINIT)
	// CID 25399: Uninitialized scalar variable (UNINIT)
	// CID 25400: Uninitialized scalar variable (UNINIT)
	// CID 25401: Uninitialized scalar variable (UNINIT)
	// CID 25402: Uninitialized scalar variable (UNINIT)
	int i = 0, intVal = 0, intVal2 = 0, encrypt = 0, enable1x = 0, wep = 0, mode = 0/*, enable1xVxd*/;
	unsigned char buf1[1024] = {0}, buf2[1024] = {0}, mac[6] = {0};
#else
...
#ifdef AEI_COVERITY_FIX
	// CID 25397: Uninitialized scalar variable (UNINIT)
	...
	if (vwlan_idx == 0) {	
		...
		pmib->dot11StationConfigEntry.dot11RegDomain = intVal;
                //turn off root SSID by default
                pmib->miscEntry.func_off = 1;
	}
...
#if 1
#ifdef AEI_COVERITY_FIX
	// CID 25403: Uninitialized scalar variable (UNINIT)
	int intVal3 = 0;
#else
	int intVal3;
#endif
...
#ifdef AEI_COVERITY_FIX
			// CID 25105: Resource leak (RESOURCE_LEAK)
			close(skfd);
#endif
			return -1;
		}
...
#endif
{
	FILE *fd;
#ifdef AEI_COVERITY_FIX
	// CID 25389: Uninitialized scalar variable (UNINIT)
	char tmpbuf[200] = {0}, buf1[100] = {0}, buf2[100] = {0};
	...
	fputs(tmpbuf, fd);
#ifdef AEI_COVERITY_FIX
	// CID 25104: Resource leak (RESOURCE_LEAK)
	fclose(fd);
#endif
...
static int updateWscConf(char *in, char *out, int genpin, char *wlanif_name)
{
	...
	//int intVal2;
#ifdef AEI_COVERITY_FIX
	// CID 25411: Uninitialized scalar variable (UNINIT)
	// CID 25413: Uninitialized scalar variable (UNINIT)
	// CID 25414: Uninitialized scalar variable (UNINIT)
	// CID 25425: Uninitialized scalar variable (UNINIT)
	int is_client = 0, is_config = 0, is_registrar = 0, len = 0, is_wep=0, wep_key_type=0, wep_transmit_key=0;
	char tmpbuf[100] = {0}, tmp1[100] = {0};
#else
...
#ifdef AEI_COVERITY_FIX
	// CID 25416: Uninitialized scalar variable (UNINIT)
	// CID 25417: Uninitialized scalar variable (UNINIT)
	// CID 25418: Uninitialized scalar variable (UNINIT)
	// CID 25419: Uninitialized scalar variable (UNINIT)
	// CID 25420: Uninitialized scalar variable (UNINIT)
	// CID 25421: Uninitialized scalar variable (UNINIT)
	// CID 25422: Uninitialized scalar variable (UNINIT)
	// CID 25423: Uninitialized scalar variable (UNINIT)
	// CID 25424: Uninitialized scalar variable (UNINIT)
	char wlan_wep64_key1[100] = {0}, wlan_wep64_key2[100] = {0}, wlan_wep64_key3[100] = {0}, wlan_wep64_key4[100] = {0};
	char wlan_wep128_key1[100] = {0}, wlan_wep128_key2[100] = {0}, wlan_wep128_key3[100] = {0}, wlan_wep128_key4[100] = {0};
	char wlan_wpa_psk[100] = {0};
#else
...
#endif
#ifdef AEI_COVERITY_FIX
	// CID 25410: Uninitialized scalar variable (UNINIT)
	// CID 25412: Uninitialized scalar variable (UNINIT)
	// CID 25415: Uninitialized scalar variable (UNINIT)
	char wlan_ssid[100] = {0}, device_name[100] = {0}, wsc_pin[100] = {0};
#else
...
#define WSC_WPA2_AES		8
	...
	/*for detial mixed mode info*/
#ifdef AEI_COVERITY_FIX
	// CID 25409: Uninitialized scalar variable (UNINIT)
	int band_select_5g2g = 0;  // 0:2.4g  ; 1:5G   ; 2:both
#else
...
#ifdef VAP_WPS
		if (!wlan0_wsc_disabled && is_vap) {
			int root_wlan_disabled;
			vwlan_idx = 0;
			apmib_get(MIB_WLAN_WLAN_DISABLED, (void *)&root_wlan_disabled); 
			if (root_wlan_disabled || wlan0_wlan_disabled)		
...
#ifdef VAP_WPS
	if (!wlan1_wsc_disabled && is_vap) {
		int root_wlan_disabled;
		vwlan_idx = 0;
		apmib_get(MIB_WLAN_WLAN_DISABLED, (void *)&root_wlan_disabled);	
		if (root_wlan_disabled || wlan1_wlan_disabled)		
	...
	if (read(fh, ptr, status.st_size) != status.st_size) {		
		printf("read() error [%s]!\n", in);
#ifdef AEI_COVERITY_FIX
		// CID 25111: Resource leak (RESOURCE_LEAK)
		close(fh);
#endif
		return -1;	
	}
#ifdef AEI_COVERITY_FIX
	// CID 25155: String not null terminated (STRING_NULL)
...
{	
	...
	flash_read_conf((char *)&compHeader, offset, sizeof(COMPRESS_MIB_HEADER_T));	
	...
	) //check whether compress mib data
	{
		...
		flash_read_conf((char *)compPtr, offset, compLen+sizeof(COMPRESS_MIB_HEADER_T));
...
#ifdef AEI_COVERITY_FIX
// CID 25100: Resource leak (RESOURCE_LEAK)
// CID 25101: Resource leak (RESOURCE_LEAK)
// CID 25099: Resource leak (RESOURCE_LEAK)
			if(compPtr != NULL)
				free(compPtr);
			if(expPtr != NULL)
				free(expPtr);
...
int flash_mib_compress_write(CONFIG_DATA_T type, char *data, PARAM_HEADER_T *pheader, unsigned char *pchecksum)
{
	...
	if(compPtr != NULL && expPtr!= NULL)
	{
		...
		if ( flash_write_conf((char *)compPtr, dst, compLen+sizeof(COMPRESS_MIB_HEADER_T))==0 )
...
#ifdef AEI_COVERITY_FIX
// CID 25103: Resource leak (RESOURCE_LEAK)
// CID 25102: Resource leak (RESOURCE_LEAK)
			if(compPtr != NULL)
				free(compPtr);
			if(expPtr != NULL)
				free(expPtr);
...
#ifdef AEI_COVERITY_FIX
		// CID 25404: Uninitialized scalar variable (UNINIT)
...
#ifdef AEI_COVERITY_FIX
	// CID 25145: Argument cannot be negative (REVERSE_NEGATIVE)
	if(client_socket < 0)
	{
...
#ifdef AEI_COVERITY_FIX
		// CID 25110: Resource leak (RESOURCE_LEAK)
		close(client_socket);
#endif
		return -1;
	}
