...
function logout()
{
	...
	pop_up("Do you really want to logout?", 1, function(){
...
function changeLongString(str, WORDS)
{
	var newstr = "";		
	...
	else if(str.length > WORDS)
	{
		for(var i = 0; i <= str.length; i += WORDS)
		{	
			newstr += str.substring(i, ((i + WORDS) > str.length ? str.length : (i + WORDS))) 
			       + ((i + WORDS) > str.length ? "" : "<br>");			
		}		
		return newstr;
	}
	else
