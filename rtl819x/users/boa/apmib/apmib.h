...
#define CWMP_NOTIFY_LIST_LEN (1024*2) //No limitation in spec
...
#define MIB_MP_SERIAL_NUM       2241

#if defined(AEI_WECB)
#define MIB_WLAN_UAPSD_ENABLED  2242

/* Store the default value of WPS PIN
   current values saved in HW_WLAN0_WSC_PIN, HW_WLAN1_WSC_PIN
 */
#define MIB_WLAN_WSC_PIN        2243
#endif
...
#define FLASH_SAVE_CONFIG		("/mnt/rt_conf/config.bin")
#else
...
#define CURRENT_SETTING_SECTOR_LEN	(CONFIG_RTL_CURRENT_SETTING_END-CONFIG_RTL_CURRENT_SETTING_OFFSET)
#else
...
#define AEI_FLASH_SIGNATURE ((char *)"aeit")
#endif
...
typedef enum { DHCP_DISABLED=0, DHCP_CLIENT=1, DHCP_SERVER=2, PPPOE=3, PPTP=4, L2TP=6, DHCP_AUTO=15 , USB3G=16, DHCP_AUTO_WAN=19 } DHCP_T; /* # keith: add l2tp support. 20080515 */
...
{
    unsigned char signature[SIGNATURE_LEN]; // should be "aeit"
    ...
    char reserved[AEI_FLASH_DATA_SIZE - 2 - SOFTWARE_VERSION_MAX -2 -2 - SIGNATURE_LEN]; // unused
} AEI_FLASH_DATA_T;
...
#if defined(AEI_WECB)
int act_rescure_wsc_pin();
int act_reset_wsc_pin();
#endif
