...
int setPSK(char *tmpBuf, char *strVal, ENCRYPT_T encrypt, int defaultKey)
{
	char key[30] = {0};
	int wep = WEP64;
	...
	else if(encrypt == ENCRYPT_WPA || encrypt == ENCRYPT_WPA2 || encrypt == ENCRYPT_WPA2_MIXED)
	{
		...
		if (!apmib_set(MIB_WLAN_WPA_AUTH, (void *)&wpaAuth)) {
...
int applyBasic(request *wp, char *tmpBuf, int both)
{
	...
	if(wlan_idx == 1)
	{
		...
		strVal = req_get_cstream_var(wp, ("wl2_ssid"), "");
		if(strVal[0])
		{
			kinfo("wl2_ssid: %s", strVal);		
			if(!setSSID(tmpBuf, strVal))
				goto setErr_basic;
		}
		...
		if(strVal[0])
		{
			...
			if(!setPSK(tmpBuf, strVal, encrypt, 0))
				goto setErr_basic;
		}	
	...
	if(wlan_idx == 0)
	{
		...
		strVal = req_get_cstream_var(wp, ("wl5_ssid"), "");
		if(strVal[0])
		{
			kinfo("wl5_ssid: %s", strVal);
			if(!setSSID(tmpBuf, strVal))
				goto setErr_basic;
		}
...
int applySecurity(request *wp, char *tmpBuf)
{
	...
	if(strVal[0])
	{
		...
		if(defaultKey == 0)
		{
			if(!setPSK(tmpBuf, strVal, encrypt, defaultKey))
				goto setErr_security;
		}
	}
	...
	else if(authentication == 2)
	{
		...
		if(strVal[0])
		{
			...
			if(!setPSK(tmpBuf, strVal, encrypt, -1))
				goto setErr_security;
		}
	}
