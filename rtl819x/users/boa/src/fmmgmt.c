...
static unsigned char *fwVersion = SOFTWARE_VERSION;
...
#if defined(AEI_WECB) && defined(AEI_ENABLE_DOWNGRADE)
char  *firmware_data_back = NULL;
int firmware_len_back = 0;
char downgrade_msg[100] = {0};	
int confirm_downgrade = 0;
...
void formUploadConfig(request *wp, char *path, char *query)
{
	...
	if (head_offset == -1) {
		strcpy(tmpBuf, "Invalid file format0!");
		goto back;
	}
	if(
#ifdef COMPRESS_MIB_SETTING
...
#ifdef ACTIONTEC_WCB
		pStart = strstr(upload_data, MACIE5_CFGSTR);
		if(pStart == NULL) {
#endif
...
{
	//fprintf(stdout,"\n@@enter firmware upload_len=%d,upload_data=%s\n",upload_len,upload_data);
	int head_offset=0 ;
	int isIncludeRoot=0;
	int		 len;
	int          locWrite;
	...
	if (head_offset == -1) {
		strcpy(buffer, "Invalid file format8!");
		goto ret_upload;
	}
	...
	while ((head_offset+sizeof(IMG_HEADER_T)) < upload_len) {
		...
		if ( fh == -1 ) {
			...
			if (numWrite < numLeft) {
				...
				// CID 25085: Resource leak (RESOURCE_LEAK)
				close(fh);
#endif
				goto ret_upload;
			}
...
static int aei_get_image_header(IMG_HEADER_T *pHeader, unsigned long bank, unsigned long *addr)
{
    int ret = -1;
    int fd;
    unsigned long offset;

    offset = CONFIG_RTL_CODE_IMAGE_OFFSET;
    ...
    fd = open(Kernel_dev_name[bank-1], O_RDWR);
    if(fd < 0)
    {
        return fd;
    }
    ...
    if(read(fd, pHeader, sizeof(IMG_HEADER_T)) != sizeof(IMG_HEADER_T))
    {
        goto DONE;
    }
...
int aei_get_flash_data(AEI_FLASH_DATA_T *addr, unsigned long image)
{
    IMG_HEADER_T bank1Header, bank2Header;
    unsigned long bank1Addr, bank2Addr;
    unsigned long bank1Mark, bank2Mark;
    int bank;
    int ret;
    int fd;
    unsigned int offset = 0;

    if(addr == NULL || ((image != BOOT_IMAGE) && (image != BACK_IMAGE)))
    {
        printf("Error parameter\n");
        return -1;
    }
    ret = aei_get_image_header(&bank1Header, 1, &bank1Addr);
    if(ret < 0)
    {
        printf("Haven't found bank1 kernel image\n");
        return -1;
    }
    bank1Mark = header_to_mark(ret, &bank1Header);

    ret = aei_get_image_header(&bank2Header, 2, &bank2Addr);
    if(ret < 0)
    {
        printf("Haven't found bank2 kernel image\n");
        return -1;
    }
	bank2Mark = header_to_mark(ret, &bank2Header);

    if(image == BOOT_IMAGE)
    {
        if(bank1Mark > bank2Mark)
        {
            offset = 0;
            bank = 1;
        }
    ...
    fd = open(flash_data_dev_name[bank-1], O_RDWR);
    if(fd < 0)
    {
        return -1;
    }
    ...
    if(read(fd, addr, sizeof(AEI_FLASH_DATA_T)) != sizeof(AEI_FLASH_DATA_T))
    {
        close(fd);
        return -1;
    }
    ...
    if(memcmp(addr->signature, AEI_FLASH_SIGNATURE, SIGNATURE_LEN) != 0)
    {
        printf("Haven't found flash data\n");
        return -1;
    }
...
int aei_swap_img(void)
{
    ...
    if(ret < 0)
    {
        printf("Haven't found bank1 kernel image, can't swap Image\n");
        return 0;
    }
    bank1Mark = header_to_mark(ret, &bank1Header);
    ...
    if(ret < 0)
    {
        printf("Haven't found bank2 kernel image, can't swap Image\n");
        return 0;
    }
	bank2Mark = header_to_mark(ret, &bank2Header);
    ...
    {
        // to active bank2
        bank2Header.burnAddr = bank1Mark + 1;
        burnAddr = bank2Addr;
        burnHeader = &bank2Header;
        backup_bank = 2;
    }
    else
    {
        // to active bank1
        bank1Header.burnAddr = bank2Mark + 1;
        burnAddr = bank1Addr;
        burnHeader = &bank1Header;
        backup_bank = 1;
    }

    fd = open(Kernel_dev_name[backup_bank-1], O_RDWR);
    if(fd < 0)
    {
        return -1;
    }

    if(lseek(fd, burnAddr, SEEK_SET) != burnAddr)
    {
        ret = -1;
        close(fd);
        return ret;
    ...
    if(write(fd, burnHeader, sizeof(IMG_HEADER_T)) != sizeof(IMG_HEADER_T))
    {
        ret = -1;
        close(fd);
        return ret;
...
/* compare_version: if(old_version < new_version) return 1,
*  if (old_version > new_version) return -1,
*  else return 0
*/
static int compare_version(const char * old_version, const char * new_version)
{
	int ret = 0;
	int old_num, new_num;
	int i = 0;
...
void formUpload(request *wp, char * path, char * query)
{
	...
	//int pid;
#endif
	int head_offset=0, tmp_offset = 0;
	int update_fw=0, update_cfg=0;
	//Support WAPI/openssl, the flash MUST up to 4m
...
#ifdef ACTIONTEC_WCB
	char error_check_url[32] = {0}; 

    char upgrade_manage[8] = {0}; 
    if(tr69GetUnfreshLeafData(upgrade_manage, "Device.ManagementServer", "UpgradesManaged"))
    {
        strcpy(error_buf, "Get Device.ManagementServer.UpgradesManaged Error!");
	    printf("ERROR: %s\n", error_buf);        
    }
    else
    {
        kinfo("upgrade_manage: %s", upgrade_manage);   

        if(!safe_strcmp(upgrade_manage, "1"))
        {   
            strcpy(error_buf, "Upgrade Firmware is locked!");
    	    printf("ERROR: %s\n", error_buf);
            goto ret_upload;
        }
    }
	...
	//fprintf(stdout,"####%s:%d wp->upload_data=%s#\n",  __FILE__, __LINE__ , wp->upload_data);
	if(strstr(wp->upload_data,"downgrade_ok"))
	{
		down_grade = "ok";
	}
	else if(strstr(wp->upload_data,"downgrade_apply"))
	{
		down_grade = "apply";
	}
	else if(strstr(wp->upload_data,"downgrade_cancel"))
	{
		down_grade = "cancel";
	}
	else
	...
	strcpy(downgrade_msg, "");
	confirm_downgrade = 0;
	if(!safe_strcmp(down_grade, "apply"))
	{
		//fprintf(stdout,"@@wp->upload_len=%d\n",wp->upload_len);
		if( firmware_data_back ) {
			free(firmware_data_back);
			firmware_data_back = NULL;
		}
		
		firmware_data_back =(char *)malloc(wp->upload_len);

		if(NULL == firmware_data_back)
			return;
		memcpy(firmware_data_back,wp->upload_data,wp->upload_len);
		
		firmware_len_back = wp->upload_len;
		//fprintf(stdout,"@@firmware_len_back=%d,firmware_data_back=%s\n",firmware_len_back,firmware_data_back);
	}
	else if(!safe_strcmp(down_grade, "ok"))
	{
		confirm_downgrade = 1;
		if( wp->upload_data ) {
			free( wp->upload_data );
		...
		//fprintf(stdout,"@@6666firmware_len_back=%d\n",firmware_len_back);
		wp->upload_data = (char *)malloc(firmware_len_back);

		if(NULL == wp->upload_data)
			return;
		
		memcpy(wp->upload_data,firmware_data_back,firmware_len_back);
		wp->upload_len = firmware_len_back;
		
		firmware_data = wp->upload_data;
		firmware_len = firmware_len_back;
		//fprintf(stdout,"@@firmware_len=%d@wp->upload_data=%s\n",firmware_len,wp->upload_data);
	...
	else if(!safe_strcmp(down_grade, "cancel"))
	{
		if(firmware_data_back) {
			free(firmware_data_back);
			firmware_data_back = NULL;
		...
		sprintf(error_check_url, "/adv_upgrade.html");	
		send_redirect_perm(wp, error_check_url);
		return;
	}
#endif		
	...
	head_offset = find_head_offset((char *)wp->upload_data);
	//fprintf(stdout,"####%s:%d content_length=%s###contenttype=%s###\n",  __FILE__, __LINE__ ,wp->content_length , wp->content_type);
	if (head_offset == -1) {
#ifdef ACTIONTEC_WCB
		strcpy(error_buf, "Invalid file format!");
		printf("ERROR: %s1\n", error_buf);
#else
		strcpy(tmpBuf, "<b>Invalid file format1!");
#endif
		goto ret_upload;
	}

	//  check Actiontec's img head and change head_offset	
	allHeader = (FW_HEADER_T *) &wp->upload_data[head_offset];
	if (memcmp(allHeader->signature, ALL_HEADER, SIGNATURE_LEN))
	{
		...
		printf("ERROR: %s2\n", error_buf);
#else
		strcpy(tmpBuf, "<b>Invalid file format2!");
#endif
		goto ret_upload;
	}

	if(!fwChecksumOk(&wp->upload_data[head_offset], sizeof(FW_HEADER_T)))
	{
#ifdef ACTIONTEC_WCB
		strcpy(error_buf, "Invalid file format!");
		printf("ERROR: %s3\n", error_buf);
	...
	if(!fwChecksumOk(&wp->upload_data[head_offset], allHeader->len))
	{
#ifdef ACTIONTEC_WCB
		strcpy(error_buf, "Invalid file format!");
		printf("ERROR: %s4\n", error_buf);
	...
	{
#ifdef ACTIONTEC_WCB
		strcpy(error_buf, "Invalid HW version!");
		printf("ERROR: %s\n", error_buf);
#else
		strcpy(tmpBuf, "<b>Invalid HW version");
#endif
		goto ret_upload;
	}
	...
	{
		int ret;
		ret = compare_version(fwVersion, (const char *)allHeader->version);
		if(ret < 0)
		{
#ifdef AEI_ENABLE_DOWNGRADE
			if(!safe_strcmp(down_grade, "apply"))
			{
				strcpy(downgrade_msg, "Downgrading will erase all user-defined settings.Do you want to continue?");
				printf(" %s\n", downgrade_msg);
				sprintf(error_check_url, "/adv_upgrade.html?e=2");	
				send_redirect_perm(wp, error_check_url);
				return;
			}
...
#ifdef ACTIONTEC_WCB
			strcpy(error_buf, "Invalid software version!");
			printf("ERROR: %s\n", error_buf);
#else
			strcpy(tmpBuf, "<b>Invalid software version");
#endif
			goto ret_upload;
#endif
		}
	...
	if (head_offset == -1)
	{
		...
		printf("ERROR: %s5\n", error_buf);
#else
		strcpy(tmpBuf, "<b>Invalid file format5!");
#endif
		goto ret_upload;
	}
    ...
    while( (tmp_offset + sizeof(IMG_HEADER_T)) <  wp->upload_len)
    {
        pHeader = (IMG_HEADER_Tp) &wp->upload_data[head_offset];

        if((pHeader->burnAddr < CONFIG_BOOT_CODE_SIZE) && memcmp(pHeader->signature, BOOT_HEADER, SIGNATURE_LEN))
        {
            strcpy(tmpBuf, "<b>This image will recover bootcoder, plese double check!");
            goto ret_upload;
        }

        tmp_offset += pHeader->len + sizeof(IMG_HEADER_T);
    }
#endif
	...
	while ((head_offset+sizeof(IMG_HEADER_T)) <  wp->upload_len) {
		...
		if (!memcmp(&wp->upload_data[head_offset], FW_HEADER, SIGNATURE_LEN) ||
			...
			isIncludeRoot = 1;	
		} else if (!memcmp(&wp->upload_data[head_offset], BOOT_HEADER, SIGNATURE_LEN)) {
			isValidfw = 1;
			flag = 4;
		} else if (
#ifdef COMPRESS_MIB_SETTING
			...
			printf("ERROR: %s6\n", error_buf);
#else			
			strcpy(tmpBuf, "<b>Invalid file format6!");
#endif
			goto ret_upload;
		}
		...
		if (len > fwSizeLimit) { //len check by sc_yang
#ifdef ACTIONTEC_WCB
			sprintf(error_buf, "Image len exceed max size 0x%x ! len=0x%x", fwSizeLimit, len);
			printf("ERROR: %s\n", error_buf);
#else			
		...
		if ( (flag == 1) || (flag == 3) || (flag == 4)) {
...
#ifdef ACTIONTEC_WCB
				sprintf(error_buf, "Image checksum mismatched! len=0x%x, checksum=0x%x", len, *((unsigned short *)&wp->upload_data[len-2]));
				printf("ERROR: %s\n", error_buf);
#else
			...
			if ( !CHECKSUM_OK((unsigned char *)ptr, len) ) {
#ifdef ACTIONTEC_WCB
				sprintf(error_buf, "Image checksum mismatched! len=0x%x", len);
				printf("ERROR: %s\n", error_buf);
#else
...
#ifdef REBOOT_CHECK
#ifdef ACTIONTEC_WCB
	sprintf(lastUrl, "%s", "/wcb_home.html");
#else
...
ret_upload:
	Reboot_Wait=0;
#ifdef ACTIONTEC_WCB	
	sprintf(error_check_url, "/adv_upgrade.html?e=1");	
	send_redirect_perm(wp, error_check_url);	
#else
	ERR_MSG(tmpBuf);
#endif
