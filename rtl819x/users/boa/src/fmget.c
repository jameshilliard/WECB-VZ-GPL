...
extern int client_select_id;
extern char *client_ssid;
extern int client_channel;
extern int client_encrypt;
extern int client_wpa_tkip_aes;
extern int client_wpa2_tkip_aes;
#endif

extern char error_buf[];
#ifdef AEI_ENABLE_DOWNGRADE
extern char downgrade_msg[];
#endif
extern int both_freq;
extern int curr_ssid;
#endif
...
int getInfo(request *wp, int argc, char **argv)
{
...
#ifdef ACTIONTEC_WCB
	else if(!strcmp(name, "currentPage"))
	{		
		return req_format_write(wp, "%s", currentPage);
	}
	else if(!strcmp(name, "countDownTitle"))
	{
		return req_format_write(wp, "%s", countDownTitle);
	}	
	else if(!strcmp(name, "errorBuf"))
	{
		return req_format_write(wp, "%s", error_buf);
	}
#ifdef AEI_ENABLE_DOWNGRADE
	else if(!strcmp(name, "downgrade_msg"))
	{
		return req_format_write(wp, "%s", downgrade_msg);
	}
#endif
	else if(!strcmp(name, "macAddress"))
	{		
		*((char *)&mac_entry) = (char)1;
		
		if (!apmib_get(MIB_WLAN_MACAC_ADDR, (void *)&mac_entry))
			return -1;
...
#ifdef ACTIONTEC_CLIENT
	else if(!strcmp(name, "clientSSID"))
	{		
		return req_format_write(wp, "%s", client_ssid);
	}
#endif
	else if(!strcmp(name, "username"))
	{
		...
		if(hs != NULL)
			return req_format_write(wp, "%s", hs->username);
		else 
	...
	else if ( !strcmp(name, "redirect_ip"))
	{
		...
		else
		{
			...
			*/
            ...
            char ip_str[20] = {0};
            char default_ip[20] = {0};

            if(tr69GetUnfreshLeafData(ip_str, "Device.DHCPv4.Client.1", "IPAddress"))
            {
                printf("Get Device.DHCPv4.Client.1.IPAddress error!\n");      
            }

            if(!safe_strcmp(ip_str, "NULL") || !safe_strcmp(ip_str, "0.0.0.0"))
            {            
                if(tr69GetUnfreshLeafData(default_ip, "Device.DHCPv4.Client.1", "X_ACTIONTEC_COM_DefaultIPAddress"))
                {
                    printf("Get Device.DHCPv4.Client.1.X_ACTIONTEC_COM_DefaultIPAddress error!\n");                
                } 
                return req_format_write(wp, "%s", default_ip);
            }
            else
                return req_format_write(wp, "%s", ip_str);			
#else
			/*
			if ( getInAddr(BRIDGE_IF, IP_ADDR, (void *)&intaddr ) )
				return req_format_write(wp, "%s", inet_ntoa(intaddr) );
...
#else
#ifdef ACTIONTEC_WCB
		req_format_write(wp, "wcb_home.html");
#else
...
int getIndex(request *wp, int argc, char **argv)
{
...
#ifdef ACTIONTEC_WCB
	else if(!strcmp(name, "wepDefaultKey"))
	{
		int wepDefaultKey;
		if(!apmib_get(MIB_WLAN_WEP_DEFAULT_KEY, (void *)&wepDefaultKey))
			return -1;
		return req_format_write(wp, "%d", wepDefaultKey);
	}	
	else if(!strcmp(name, "wifi_certificated"))
	{
...
#ifdef ACTIONTEC_CLIENT
	else if(!strcmp(name, "clientSelectID"))
	{
		return req_format_write(wp, "%d", client_select_id);
	}
	else if(!strcmp(name, "clientChannel"))
	{
		return req_format_write(wp, "%d", client_channel);
	}
	else if(!strcmp(name, "clientEncrypt"))
	{
		return req_format_write(wp, "%d", client_encrypt);
	}
	else if(!strcmp(name, "clientWpaTkipAes"))
	{
		return req_format_write(wp, "%d", client_wpa_tkip_aes);
	}
	else if(!strcmp(name, "clientWpa2TkipAes"))
	{
		return req_format_write(wp, "%d", client_wpa2_tkip_aes);
	}
#endif
	else if(!strcmp(name, "auth_level"))
	{
#ifdef ACTIONTEC_MULTIACCESS
		hs = er_http_session_lookup(wp->session_id);

		if(hs != NULL)
			return req_format_write(wp, "%d", hs->auth_level);
		else
			return req_format_write(wp, "0");
#else
		return req_format_write(wp, "0");
	...
	else if(!strcmp(name, "wifi_supported"))
	{
#ifdef ACTIONTEC_MULTIACCESS
		hs = er_http_session_lookup(wp->session_id);

		if(hs != NULL)
			return req_format_write(wp, "%d", hs->wifi);
		else
			return req_format_write(wp, "0");
#else
		return req_format_write(wp, "0");
	...
	else if(!strcmp(name, "both_freq"))
	{
#ifdef ACTIONTEC_MULTIACCESS
	hs = er_http_session_lookup(wp->session_id);
		
	if(hs != NULL)
		return req_format_write(wp, "%d", hs->both_freq);
	else
		return req_format_write(wp, "%d", both_freq);
#else
	return req_format_write(wp, "%d", both_freq);
#endif	
	}	
	else if(!strcmp(name, "curr_ssid"))
	{
#ifdef ACTIONTEC_MULTIACCESS
	hs = er_http_session_lookup(wp->session_id);
				
	if(hs != NULL)
		return req_format_write(wp, "%d", hs->curr_ssid);
	else
		return req_format_write(wp, "%d", curr_ssid);
#else
	return req_format_write(wp, "%d", curr_ssid);
#endif
	}	
...
//		char tmpStr[10];
	...
	if(hs != NULL)
		return req_format_write(wp, "%d", hs->wlan_idx);
	else
		return req_format_write(wp, "%d", wlan_idx);
#else
