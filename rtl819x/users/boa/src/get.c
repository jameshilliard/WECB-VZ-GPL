...
int init_get2(request * req)
{
	...
	if(hs!=NULL)
	{
		if(strstr(hs->username,"user")&&(strstr(req->request_uri, "advtr69")||strstr(req->request_uri, "adv_tr69")))
		{
			log_error_doc(req);
...
#if !defined(AEI_TELNETD)
	if(strcmp(req->pathname, "/web/telnetd_start")==0)
	{
		//fprintf(stdout,"\n@will telnet@\n");
		system("telnetd &");
		//fprintf(stdout,"\n@end telnet@\n");
		send_redirect_perm(req, MAIN_PAGE);
		return 0;
	}
#endif
