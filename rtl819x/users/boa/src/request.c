...
#ifdef ACTIONTEC_WCB
			if(isFWUPGRADE == 1 && 
				((!strcmp(current->request_uri,"/formUpload.cgi")) 
				   || (!strcmp(current->request_uri,"/boafrm/formUpload")))) {
#else
...
#ifdef ACTIONTEC_MULTIACCESS
{
	...
	if(strstr(req->request_uri, "userlogout.cgi")) 
	{
		//printf("user logout\n");
		er_http_session_remove(req->session_id);		
	}

	if(!strstr(req->request_uri, "wcb_home.html") 
		&& !strstr(req->request_uri, "index.html")		
		&& !strstr(req->request_uri, "ajax_")
		&& strstr(req->request_uri, ".html"))
	{		
		kdebug("req->session_id: %u", req->session_id);
		hs = er_http_session_lookup(req->session_id);

		if(hs == NULL)
		{				
			if ((last_request_uri = (char *) malloc(strlen(req->request_uri) + 1)) != NULL)
			{
				memset(last_request_uri, 0, strlen(req->request_uri) + 1);
				strncpy(last_request_uri, req->request_uri, strlen(req->request_uri)); 
			}			
			
			kdebug("session is null");		
			send_redirect_perm(req, "/wcb_home.html");
		}
		else
		{									
			...
			/*
			printf("start_time: %u, cur_time: %u, diff_time: %u, username: %s, auth_level: %d\n", 
				hs->start_time, cur_time, (cur_time - hs->start_time), hs->username, hs->auth_level);			
			*/

			kdebug("time_diff: %u, cur_time: %u, start_time: %u", (cur_time - hs->start_time), cur_time, hs->start_time);
			
			if((cur_time - hs->start_time) > SESSION_TIME)									
			{			
				kdebug("session time exceed %d", SESSION_TIME);
				send_redirect_perm(req, "/wcb_home.html");										
				er_http_session_remove(req->session_id);
			}
			else 
			{
				if(hs->wifi == 0 && strstr(req->request_uri, "wl_"))
					send_redirect_perm(req, "/wcb_home.html?k=5");	

				kdebug("request_uri: %s, auth_level: %d", req->request_uri, hs->auth_level);
				if(!get_perm(req->request_uri, hs->auth_level))
				{		
					send_redirect_perm(req, "/wcb_home.html?k=3");
				}		
				
				hs->start_time = cur_time;
			}
		}	
	}
}	
