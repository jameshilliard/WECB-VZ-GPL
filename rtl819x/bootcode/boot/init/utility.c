...
int sc16is7x0_err = -1;
#define UART_MEM2REG( m )	( ( ( m ) - UART_RBR_REG ) / 4 )
#endif
...
int RTL_TIMEOUT()
{
    ...
    // CID 29557: Missing return statement (MISSING_RETURN)
    return 0;
#endif
}
...
static hw_led_t aei_comcast_wcb3000_hw_led[] = {
    {COMCAST_WCB3000_POWER_WHITE_LED, LED_ON, STATE_NOSWAP},
    {COMCAST_WCB3000_POWER_RED_LED, LED_ON, STATE_NOSWAP},
    {COMCAST_WCB3000_MOCA_LED, LED_ON, STATE_SWAP},
    {COMCAST_WCB3000_WPS_WHITE_LED, LED_ON, STATE_NOSWAP},
    {COMCAST_WCB3000_WPS_RED_LED, LED_ON, STATE_NOSWAP},
    {COMCAST_WCB3000_WIFI2G_LED, LED_ON, STATE_NOSWAP},
    {COMCAST_WCB3000_WIFI5G_LED, LED_ON, STATE_NOSWAP},
    {-1, 0},
};

static hw_led_t aei_comcast_ecb3000_hw_led[] = {
    {COMCAST_ECB3000_POWER_RED_LED, LED_OFF, STATE_NOSWAP},
    {-1, 0},
};

static hw_led_t aei_ncs_wcb3000_hw_led[] = {
    {NCS_WCB3000_POWER_GREEN_LED, LED_ON, STATE_NOSWAP},
    {NCS_WCB3000_POWER_RED_LED, LED_ON, STATE_NOSWAP},
    {NCS_WCB3000_MOCA_LED, LED_OFF, STATE_SWAP},
    {NCS_WCB3000_WPS_GREEN_LED, LED_ON, STATE_NOSWAP},
    {NCS_WCB3000_WPS_RED_LED, LED_ON, STATE_NOSWAP},
    {NCS_WCB3000_WIFI2G_LED, LED_OFF, STATE_NOSWAP},
    {NCS_WCB3000_WIFI5G_LED, LED_OFF, STATE_NOSWAP},
    {-1, 0},
};

static hw_led_t aei_ncs_ecb3000_hw_led[] = {
    {NCS_ECB3000_POWER_RED_LED, LED_OFF},
    {-1, 0},
};


// boardID must same as kernel boardid
AEI_BOARD_INFO_T aei_board_info[] = {
    [AEI_PRODUCT_COMCAST_WCB3000] = {AEI_BOARD_ID_COMCAST_WCB3000, aei_comcast_wcb3000_hw_led},
    [AEI_PRODUCT_COMCAST_ECB3000] = {AEI_BOARD_ID_COMCAST_ECB3000, aei_comcast_ecb3000_hw_led},
    [AEI_PRODUCT_NCS_WCB3000] = {AEI_BOARD_ID_NCS_WCB3000, aei_ncs_wcb3000_hw_led},
    [AEI_PRODUCT_NCS_ECB3000] = {AEI_BOARD_ID_NCS_ECB3000, aei_ncs_ecb3000_hw_led},
};



static hw_led_t aei_comcast_wcb3000_hw_led_down_mode[] = {
    {COMCAST_WCB3000_POWER_WHITE_LED, LED_OFF, STATE_NOSWAP},
    {COMCAST_WCB3000_POWER_RED_LED, LED_ON, STATE_NOSWAP},
    {COMCAST_WCB3000_MOCA_LED, LED_OFF, STATE_SWAP},
    {COMCAST_WCB3000_WPS_WHITE_LED, LED_OFF, STATE_NOSWAP},
    {COMCAST_WCB3000_WPS_RED_LED, LED_OFF, STATE_NOSWAP},
    {COMCAST_WCB3000_WIFI2G_LED, LED_OFF, STATE_NOSWAP},
    {COMCAST_WCB3000_WIFI5G_LED, LED_OFF, STATE_NOSWAP},
    {-1, 0},
};

static hw_led_t aei_comcast_ecb3000_hw_led_down_mode[] = {
    {COMCAST_ECB3000_POWER_RED_LED, LED_ON, STATE_NOSWAP},
    {-1, 0},
};

static hw_led_t aei_ncs_wcb3000_hw_led_down_mode[] = {
    {NCS_WCB3000_POWER_GREEN_LED, LED_OFF, STATE_NOSWAP},
    {NCS_WCB3000_POWER_RED_LED, LED_ON, STATE_NOSWAP},
    {NCS_WCB3000_MOCA_LED, LED_OFF, STATE_SWAP},
    {NCS_WCB3000_WPS_GREEN_LED, LED_OFF, STATE_NOSWAP},
    {NCS_WCB3000_WPS_RED_LED, LED_OFF, STATE_NOSWAP},
    {NCS_WCB3000_WIFI2G_LED, LED_OFF, STATE_NOSWAP},
    {NCS_WCB3000_WIFI5G_LED, LED_OFF, STATE_NOSWAP},
    {-1, 0},
};

static hw_led_t aei_ncs_ecb3000_hw_led_down_mode[] = {
    {NCS_ECB3000_POWER_RED_LED, LED_ON},
    {-1, 0},
};


// boardID must same as kernel boardid
AEI_BOARD_INFO_T aei_board_info_down_mode[] = {
    [AEI_PRODUCT_COMCAST_WCB3000] = {AEI_BOARD_ID_COMCAST_WCB3000, aei_comcast_wcb3000_hw_led_down_mode},
    [AEI_PRODUCT_COMCAST_ECB3000] = {AEI_BOARD_ID_COMCAST_ECB3000, aei_comcast_ecb3000_hw_led_down_mode},
    [AEI_PRODUCT_NCS_WCB3000] = {AEI_BOARD_ID_NCS_WCB3000, aei_ncs_wcb3000_hw_led_down_mode},
    [AEI_PRODUCT_NCS_ECB3000] = {AEI_BOARD_ID_NCS_ECB3000, aei_ncs_ecb3000_hw_led_down_mode},
};

#define AEI_BOARD_INFO_SIZE_DOWN_MODE (sizeof(aei_board_info_down_mode)/sizeof(aei_board_info_down_mode[0]))

int aei_get_hw_version()
{
    static int kernel_boardid = -1;
    if(kernel_boardid < 0)
    {
        kernel_boardid = AEIGetHWVersion();
    }

    return kernel_boardid;
}

void set_all_leds_state(AEI_BOARD_INFO_T* board_info, int count,  int value)
{
    int i = 0;
    int kernel_boardid = 0;

    kernel_boardid = aei_get_hw_version();
    if(kernel_boardid < 0)
    {
        return;
    }
    ...
    for(i = 0; i < count; i++)
    {
        if(kernel_boardid == board_info->boardID)
        {           
            int iter = 0;
            while(board_info->hw_led[iter].gpio != -1)
            {
                gpio_init(&board_info->hw_led[iter]);
                led_set_state(&board_info->hw_led[iter], value);
                iter++;
            }
            break;
        }
...
/*
    The the current boardid, then decide LED behavior

    Return:
        Success: return boardid
        Fail: return -1
*/
int aei_get_boardid(void)
{
    ...
    for(i = 0; i < sizeof(aei_board_info)/sizeof(aei_board_info[0]); i++)
    {
        if(aei_board_info[i].boardID == kernel_boardid)
        {
            boardid = i;
            break;
        }
...
/*
    ...
    Return:
        Success: Return point, point to struct for board info
        Fail: Return NULL
*/
AEI_BOARD_INFO_T *aei_get_boardinfo(void)
{
    int boardid;

    boardid = aei_get_boardid();
    if(boardid < 0 || (boardid >= sizeof(aei_board_info)/sizeof(aei_board_info[0])))
    {
        return NULL;
    }

    return &aei_board_info[boardid];
}

void led_set_state(hw_led_t *hled, int value)
{
    if((hled == NULL) || (hled->gpio == -1))
    {
        return;
    }

    if(value & LED_DEFAULT)
    {
        value = hled->state;
    }

    // if HW connect not same as others LED, then need swap
    if(hled->swap & STATE_SWAP)
    {
        if(value & LED_OFF)
        {
            value = (value & (~LED_OFF)) | LED_ON;
        }
        else
        {
            value = (value & (~LED_ON)) | LED_OFF;
        }
    }
    ...
    if(hled->gpio < 32)
    {
        if(value & LED_OFF)
        {
            REG32(PABCDDAT_REG) = REG32(PABCDDAT_REG) | (1 << hled->gpio);
        }
    ...
    else if(hled->gpio < 64)
    {
        if(value & LED_OFF)
        {
            REG32(PEFGHDAT_REG) = REG32(PEFGHDAT_REG) | (1 << (hled->gpio - 32));
        }
        else if(value & LED_ON)
        {
            REG32(PEFGHDAT_REG) = REG32(PEFGHDAT_REG) & ~(1 << (hled->gpio - 32));
    ...
    {
        prom_printf("Error gpio number 0x%x\n", hled->gpio);
        return;
    }
...
static int hw_set_gpio_mode(unsigned int gpio)
{
    switch(gpio)
    {
        ...
        case GPIOA(2):
        case GPIOA(3):
            REG32(RTL_GPIO_MUX) = REG32(RTL_GPIO_MUX) | 0x3 << 3; // set jtag as GPIO
            break;
        case GPIOB(0):
            REG32(RTL_GPIO_MUX2) = REG32(RTL_GPIO_MUX2) | 0x3 << 1; // set gpios0 as GPIO
            break;
        case GPIOB(1):
            REG32(RTL_GPIO_MUX2) = REG32(RTL_GPIO_MUX2) | 0x3 << 4; // set gpios1 as GPIO
            break;
        case GPIOB(2):
            REG32(RTL_GPIO_MUX2) = REG32(RTL_GPIO_MUX2) | 0x3 << 7; // set gpios2 as GPIO
            break;
        case GPIOB(3):
            REG32(RTL_GPIO_MUX2) = REG32(RTL_GPIO_MUX2) | 0x3 << 10; // set gpios3 as GPIO
            break;
        case GPIOC(0):
            REG32(RTL_GPIO_MUX2) = REG32(RTL_GPIO_MUX2) | 0x3 << 13; // set gpiop0 as GPIO
            break;
        case GPIOC(2):
            REG32(RTL_GPIO_MUX2) = REG32(RTL_GPIO_MUX2) | 0x3 << 19; // set gpiop2 as GPIO
            break;
        case GPIOC(3):
            REG32(RTL_GPIO_MUX2) = REG32(RTL_GPIO_MUX2) | 0x3 << 22; // set gpiop3 as GPIO
            break;
        case GPIOH(0):
        case GPIOH(1):
        case GPIOH(2):
            REG32(RTL_GPIO_MUX) = REG32(RTL_GPIO_MUX) | 0x3 << 1; // set gpioh as GPIO
            break;
        default:
            prom_printf("Set GPIO Mode error\n");
            return -1;
    }
...
{
    static int init_74hct_done = 0;
    if(hled == NULL)
    {
        return;
    }

    if(init_74hct_done == 0 && (hled->gpio == COMCAST_WCB3000_WIFI2G_LED || 
        hled->gpio == COMCAST_WCB3000_WIFI5G_LED || hled->gpio == COMCAST_WCB3000_MOCA_LED))
    {
        hw_set_gpio_mode(COMCAST_WCB3000_74HCT);

        if(COMCAST_WCB3000_74HCT < 32)
        {
            REG32(PABCDCNR_REG) = REG32(PABCDCNR_REG) & ~(1 << COMCAST_WCB3000_74HCT); // Configured as GPIO pin
            REG32(PABCDDIR_REG) = REG32(PABCDDIR_REG) | (1 << COMCAST_WCB3000_74HCT); // Configured as output pin
            REG32(PABCDDAT_REG) = REG32(PABCDDAT_REG) & ~(1 << COMCAST_WCB3000_74HCT); // Set gpio value as 0
        }
        else
        {
            REG32(PEFGHCNR_REG) = REG32(PEFGHCNR_REG) & ~(1 << (COMCAST_WCB3000_74HCT - 32)); // Configured as GPIO pin
            REG32(PEFGHDIR_REG) = REG32(PEFGHDIR_REG) | (1 << (COMCAST_WCB3000_74HCT - 32)); // Configured as output pin
            REG32(PEFGHDAT_REG) = REG32(PEFGHDAT_REG) & ~(1 << (COMCAST_WCB3000_74HCT - 32)); // Set gpio value as 0
        }
    ...
    if(hled->gpio < 32)
    {
        REG32(PABCDCNR_REG) = REG32(PABCDCNR_REG) & ~(1 << hled->gpio); // Configured as GPIO pin
        REG32(PABCDDIR_REG) = REG32(PABCDDIR_REG) | (1 << hled->gpio); // Configured as output pin
    }
    else
    {
        REG32(PEFGHCNR_REG) = REG32(PEFGHCNR_REG) & ~(1 << (hled->gpio - 32)); // Configured as GPIO pin
        REG32(PEFGHDIR_REG) = REG32(PEFGHDIR_REG) | (1 << (hled->gpio - 32)); // Configured as output pin
    }
...
{
    int i = 0;
    AEI_BOARD_INFO_T *boardinfo = NULL;

    boardinfo = aei_get_boardinfo();
    if(boardinfo == NULL)
    {
        return;
    }

    while(boardinfo->hw_led[i].gpio != -1)
    {
        gpio_init(&boardinfo->hw_led[i]);
        led_set_state(&boardinfo->hw_led[i], value);
        i++;
    }
...
void set_led_state_by_gpio(AEI_BOARD_INFO_T *boardinfo, int gpio, int value)
{
    ...
    while(boardinfo->hw_led[i].gpio != -1)
    {
        if(boardinfo->hw_led[i].gpio == gpio)
        {
            gpio_init(&boardinfo->hw_led[i]);
            led_set_state(&boardinfo->hw_led[i], value);
            break;
...
{
    AEI_BOARD_INFO_T *boardinfo = NULL;
    int kernel_boardid = AEIGetHWVersion();
    boardinfo = aei_get_boardinfo();
    ...
    switch(kernel_boardid)
    {
        case AEI_BOARD_ID_COMCAST_WCB3000:
            {
                set_led_state_by_gpio(boardinfo, COMCAST_WCB3000_POWER_RED_LED, value);
                set_led_state_by_gpio(boardinfo, COMCAST_WCB3000_POWER_WHITE_LED, value);
                break;
            }
        case AEI_BOARD_ID_NCS_WCB3000 :
            {
                set_led_state_by_gpio(boardinfo, NCS_WCB3000_POWER_GREEN_LED, value);
                set_led_state_by_gpio(boardinfo, NCS_WCB3000_POWER_RED_LED, value);
                break;
            }
    }
}
...
#ifdef AEI_WECB
    // Lit all LED during Power on Self Test(POST)
    set_all_led_state(LED_DEFAULT);
#endif
...
void console_init(unsigned long lexea_clock)
{
	...
	unsigned long dlm;       
  	...
  	*(volatile unsigned long *)(0xa1000000) = dl ; 
  	...
  	//REG32(UART_LCR_REG)=0x03000000;		//Line Control Register  8,n,1
  	sc16is7x0_serial_out_i2c( UART_MEM2REG( UART_LCR_REG ), 0x03 );
		
  	//REG32( UART_FCR_REG)=0xc7000000;		//FIFO Ccontrol Register
  	sc16is7x0_serial_out_i2c( UART_MEM2REG( UART_FCR_REG ), 0xc7 );
  	//REG32( UART_IER_REG)=0x00000000;
  	sc16is7x0_serial_out_i2c( UART_MEM2REG( UART_IER_REG ), 0x00 );
		//dl = (lexea_clock /16)/BAUD_RATE-1;		
		...
		dlm = dl >> 8;
  	//REG32( UART_LCR_REG)=0x83000000;		//Divisor latch access bit=1
  	sc16is7x0_serial_out_i2c( UART_MEM2REG( UART_LCR_REG ), 0x83 );
  	//REG32( UART_DLL_REG)=dll*0x1000000;
  	sc16is7x0_serial_out_i2c( UART_MEM2REG( UART_DLL_REG ), dll );
   	//REG32( UART_DLM_REG)=dlm*0x1000000; 
   	sc16is7x0_serial_out_i2c( UART_MEM2REG( UART_DLM_REG ), dlm );
	//REG32( UART_LCR_REG)=0x83000000& 0x7fffffff;	//Divisor latch access bit=0
	sc16is7x0_serial_out_i2c( UART_MEM2REG( UART_LCR_REG ), 0x83 & 0x7f );
   	//rtl_outl( UART_THR,0x41000000);	
#endif
...
#ifndef CONFIG_FPGA_PLATFORM
#ifndef CONFIG_RTL8198
	REG32(RTL_GPIO_MUX)=REG32(RTL_GPIO_MUX)&(0xFFFFFFFF-0x00300000);	
#else

#ifdef CONFIG_USING_JTAG
	REG32(RTL_GPIO_MUX)=(0x07);
#else
	REG32(RTL_GPIO_MUX)=(0x0f);
#endif
#endif	
#endif
...
#endif
    set_all_leds_state( aei_board_info_down_mode, AEI_BOARD_INFO_SIZE_DOWN_MODE, LED_DEFAULT);
    monitor();
	return ;
}
