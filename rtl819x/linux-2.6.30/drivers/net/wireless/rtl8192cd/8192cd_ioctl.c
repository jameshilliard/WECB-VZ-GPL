...
#define SIOC92DTEMPMP	0x8BC2
#ifdef CONFIG_RTL_92D_DMDP
#define SIOC92DSMCC		0x8BC3 //smart concurrent
#endif
#ifdef EN_EFUSE
...
static struct iwpriv_arg mib_table[] = {
	...
	{"Non40MSTADeny",	INT_T,		_OFFSET(dot11StationConfigEntry.Non40MSTADeny), _SIZE(dot11StationConfigEntry.Non40MSTADeny), 0},
#ifdef CLIENT_MODE
	...
	{"stbc",		INT_T,		_OFFSET(dot11nConfigEntry.dot11nSTBC), _SIZE(dot11nConfigEntry.dot11nSTBC), 0},
...
#else
#if defined(CONFIG_RTL8196B_GW_8M) || defined(CONFIG_RTL8196C_AP_ROOT) || defined(CONFIG_RTL_8198_AP_ROOT) || defined(CONFIG_RTL8196C_CLIENT_ONLY) || defined(__ECOS)
	...
	{"edcca_thd",		RFFT_T, _OFFSET_RFFT(edcca_thd), _SIZE_RFFT(edcca_thd), 0},
...
#ifdef CONFIG_RTL8672
	{"1rcca",				RFFT_T,	_OFFSET_RFFT(one_path_cca), _SIZE_RFFT(one_path_cca), 1},
#else
...
struct _ioctl_led_status
{
    UINT led_mode;
    UINT led_interval;  
    UINT led_on_interval;
    UINT led_off_interval;
    int led_blink_cycles;
    UINT led_toggle;
    UINT led_toggle_start;
};

/**************************************************************************
 *     [FUNCTION NAME]:
 *            rtl8192cd_get_led_status
 *
 *     [DESCRIPTION]:
 *            copy wireless led info to userspace.
 *
 *     [PARAMETER]:
 ...
 *
 *     [RETURN]
 *           copy rtl8192cd_priv's led info to userspace.
 *           >= 0 Success
 *           < 0 Failed
 **************************************************************************/
static int rtl8192cd_get_led_status(struct rtl8192cd_priv *priv, UINT8* data)
{
    struct _ioctl_led_status  status = {0};

    status.led_mode = priv->pshare->LED_Mode;
    status.led_blink_cycles = priv->pshare->LED_Blink_Cycles;
    status.led_interval = priv->pshare->LED_Interval;
    status.led_on_interval = priv->pshare->LED_ON_Interval;
    status.led_off_interval = priv->pshare->LED_OFF_Interval;
    status.led_toggle = priv->pshare->LED_Toggle;
    status.led_toggle_start = priv->pshare->LED_ToggleStart;

    if (copy_to_user((void *)data, (void *)&status, sizeof(struct _ioctl_led_status)))
        return -1;
...
#ifdef CONFIG_RTL_92D_DMDP
				{ SIOC92DSMCC, IW_PRIV_TYPE_CHAR | 128, 0, "smcc" },
#endif
#ifdef EN_EFUSE
...
case SIOC_SET_ROUTING_INFO:
	...
	case SIOC92DTEMPMP:
	{
		if (wrq->u.data.pointer) {
			...
			priv->mp_txrate = _atoi(tmpbuf, 16);
			printk("MP TX rate = %d \n", priv->mp_txrate);
			ret = 0;
		} else {
			ret = -1;
	...
	{
		#if 0
		extern int smcc_92D_enable2x2_5G(struct rtl8192cd_priv * priv);
		extern int smcc_92D_enable2x2_2G(struct rtl8192cd_priv * priv);

		if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) {
			ret = smcc_92D_enable2x2_5G(priv);
			printk("switch to 2x2 5G (ret=%d)\n", ret);
		} else {
			ret = smcc_92D_enable2x2_2G(priv);
			printk("switch to 2x2 2G (ret=%d)\n", ret);
		}
		#else
...
#ifdef AEI_WECB
    // SW-Task#101588
    case SIOWIFILEDSTATUS:
        ret = rtl8192cd_get_led_status(priv, (unsigned char *)(wrq->u.data.pointer));
        break;
#endif
	}
