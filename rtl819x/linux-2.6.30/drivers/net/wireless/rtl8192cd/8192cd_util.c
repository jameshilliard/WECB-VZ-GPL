...
#ifdef CONFIG_AEI_WLAN_TX_OFFLOAD
extern void aei_wlan_tx_offload_tx_cnt_report(int difftime,int pktcnt);
extern unsigned long volatile jiffies;
unsigned long aei_wlan_tx_offload_old_jiffies = 0;
#endif
...
static struct dz_mgmt_queue *alloc_sta_mgt_que(struct rtl8192cd_priv *priv)
{
	...
	SAVE_INT_AND_CLI(flags);
#ifdef CONCURRENT_MODE
	struct dz_mgmt_queue *priv_que = (struct dz_mgmt_queue*)get_buf_from_poll(priv, &mgt_quebuf_list[priv->pshare->wlandev_idx], (unsigned int *)&free_mgt_que_buf_num[priv->pshare->wlandev_idx]);
#else
	struct dz_mgmt_queue *priv_que = (struct dz_mgmt_queue*)get_buf_from_poll(priv, &mgt_quebuf_list, (unsigned int *)&free_mgt_que_buf_num);
#endif
	RESTORE_INT(flags);
...
//yschen 2009-03-04
...
#ifdef AEI_WECB
    {
        ...
        if(!sta_num){
            set_wlan_led(priv_root, LED_MODE_ON, 0, 0, 0, 0);//Radio Enabled (No Client, No Probe)
        }
    }
#endif
}
...
#if 0	/*2012-0920 veriwave upd TP issue*/ 
...
#if 0	/*2012-1101 veriwave upd TP issue*/ 			
...
void restore_backup_sta(struct rtl8192cd_priv *priv, void *pInfo)
{
	...
	#if defined(CONFIG_RTL8196B_GW_8M) || defined(CONFIG_RTL8196C_AP_ROOT) || defined(CONFIG_RTL8196C_CLIENT_ONLY) || defined(CONFIG_RTL_8198_AP_ROOT)
#ifdef __LINUX_2_6__
...
static __inline__ unsigned char *get_priv_skb_buf(struct rtl8192cd_priv *priv)
{
	...
	unsigned char *data = get_buf_from_poll(priv, &skbbuf_list[i], (unsigned int *)&skb_free_num[i]);
...
#ifdef CONFIG_AEI_WLAN_TX_OFFLOAD
	aei_wlan_tx_offload_tx_cnt_report(jiffies - aei_wlan_tx_offload_old_jiffies,pkt_cnt);
	aei_wlan_tx_offload_old_jiffies = jiffies;
#endif
