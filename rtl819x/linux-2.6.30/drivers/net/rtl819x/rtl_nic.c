...
#endif
#define CONFIG_RTL_PREV_L2_AGING		1

#ifdef CONFIG_RTL_PREV_L2_AGING
__DRAM_FWD unsigned char shortcut_sta_addr[ETHER_ADDR_LEN] = { 0,0,0,0 };
int upd_l2_age_counter = 0;
#endif
#ifdef CONFIG_RTL_VLAN_8021Q 
int cache_dev_age_counter = 0;				/*cache dev age out time*/
__DRAM_FWD unsigned short cached_wlan_vid=0;
EXPORT_SYMBOL(cached_wlan_vid);
__DRAM_FWD struct cached_br_info cached_eth_info[CACHE_ETH_DEV_MAX];
EXPORT_SYMBOL(cached_eth_info);
#endif
#endif
...
/*		=====  ===   =======	===	 ===   =========   =======	====	====================================	*/
	...
	{ 	RTL_DRV_LAN_NETIF_NAME,	 0,   IF_ETHER, 	RTL_LANVLANID, 	   	RTL_LAN_FID, 	RTL_LANPORT_MASK_4 | RTL_LANPORT_MASK_3, 	RTL_LANPORT_MASK_4 | RTL_LANPORT_MASK_3,		1500, 	{ { 0x00, 0x12, 0x34, 0x56, 0x78, 0x90 } }, 0	},
...
#elif defined(CONFIG_AEI_HW_JNR3000)
	{	RTL_DRV_LAN_NETIF_NAME,  0,   IF_ETHER, 	RTL_LANVLANID,		RTL_LAN_FID,	RTL_LANPORT_MASK_4,	RTL_LANPORT_MASK_4,		1500,	{ { 0x00, 0x12, 0x34, 0x56, 0x78, 0x90 } }, 0	},
#endif
#else
...
#ifdef CONFIG_AEI_MOCA_ENABLE
	{	MOCA_IFNAME,	0,	 IF_ETHER,	RTL_LANVLANID,	RTL_LAN_FID,		RTL_LANPORT_MASK_5, 	RTL_LANPORT_MASK_5, 	1500,	{ { 0x00, 0x12, 0x34, 0x56, 0x78, 0x95 } }, 0	},
#endif //CONFIG_AEI_MOCA_ENABLE
...
static int act_gmac_do_ioctl (struct net_device *dev, struct ifreq *rq, int cmd);
...
#define MOCA_IFNAME     "clink0"
#if defined(CONFIG_AEI_HW_WECB3000) || defined(CONFIG_AEI_HW_JNR3000)
#define MOCA_RESET_GPIO       16
#elif defined(CONFIG_AEI_HW_ECB3000)
#define MOCA_RESET_GPIO       58
#endif



extern int rtl_gpio_out_init(int pin);
extern int rtl_gpio_out_val(int pin, int val);


int rtl_gpio_out_init(int pin)
{
	if(pin < 32)
	{
		RTL_W32(PABCD_CNR, RTL_R32(PABCD_CNR) & (~(0x1<<pin))); //set GPIO pin
		RTL_W32(PABCD_DIR, RTL_R32(PABCD_DIR) | (0x1<<pin));//output pin
	}
	else if(pin < 64)
	{
		RTL_W32(PEFGH_CNR, RTL_R32(PEFGH_CNR) & (~(0x1<<(pin - 32)))); //set GPIO pin
		RTL_W32(PEFGH_DIR, RTL_R32(PEFGH_DIR) | (0x1<<(pin - 32)));//output pin
	}
	else
	{
		return -1;
...
EXPORT_SYMBOL(rtl_gpio_out_init);

int rtl_gpio_out_val(int pin, int val)
{
	if(pin >= 0 && pin < 32)
	{
		if (val){
			RTL_W32(PABCD_DAT, (RTL_R32(PABCD_DAT) | (0x1<<pin)));//set 1
		}
		else
		{
			RTL_W32(PABCD_DAT, (RTL_R32(PABCD_DAT) & (~((0x1<<pin)))));//set 0
		}
		
		return RTL_R32(PABCD_DAT);
	}
	else if(pin >=32 && pin < 64)
	{
		if (val){
			RTL_W32(PEFGH_DAT, (RTL_R32(PEFGH_DAT) | (0x1<<(pin - 32))));//set 1
		}
		else
		{
			RTL_W32(PEFGH_DAT, (RTL_R32(PEFGH_DAT) & (~(0x1<<(pin - 32)))));//set 0
		}
		
		return RTL_R32(PEFGH_DAT);
	}

	return -1;
}
EXPORT_SYMBOL(rtl_gpio_out_val);

int rtl_gpio_in_val(int gpio)
{
	return 0;
}

extern int moca_enet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd);
extern int moca_memory_open(struct net_device *dev);
extern int moca_memory_release(struct net_device *dev);

extern struct ethtool_ops moca_ethtool_ops;
...
int  rtl_MulticastRxCheck(struct sk_buff *skb,rtl_nicRx_info *info)
{
	...
	if((skb->data[0]==0x01) && (skb->data[1]==0x00)&& (skb->data[2]==0x5e))
	{
        ...
        {
            vid = *((unsigned short *)(skb->data+(ETH_ALEN<<1)+2));
            vid = vid & 0xfff;
            skb->srcVlanId = vid;
            //panic_printk("%s %d skb->srcVlanId 0x%x\n", __FUNCTION__, __LINE__, skb->srcVlanId);
        }
        else 
        {
	        skb->srcVlanId=pvid;
...
#ifdef CONFIG_RTL_VLAN_8021Q
				if(linux_vlan_enable)
					re865x_relayTrappedMCast( skb, vid, nicMCastFwdInfo.fwdPortMask, TRUE);
				else
#endif
...
static inline int32 rtl_processBridgeShortCut(struct sk_buff *skb, struct dev_priv *cp_this, rtl_nicRx_info *info)
{
	...
	#endif /* #if defined(CONFIG_RTL_ULINKER_BRSC) */
	{
		...
		#endif
		{
			...
			#ifdef CONFIG_RTL_PREV_L2_AGING
			memcpy(shortcut_sta_addr, &skb->data[0], ETH_ALEN);
			#endif
...
static inline void rtl_processRxFrame(rtl_nicRx_info *info)
{
	...
	uint8			*data;
#ifdef CONFIG_RTL_VLAN_8021Q 
	struct net_device *cached_wlan_dev;
#endif
	cp_this = info->priv;
	skb = info->input;
	vid = info->vid;
		...
		if(*((uint16*)(skb->data+(ETH_ALEN<<1))) != __constant_htons(ETH_P_8021Q)){
			...
			*((uint16*)(skb->data+(ETH_ALEN<<1))) = __constant_htons(ETH_P_8021Q);
			if(info->srcvid){				
				...
				*((uint16*)(skb->data+(ETH_ALEN<<1)+2)) = vlan_ctl_p->pvid[info->srcpid];
			}				
		}
	}
	else
	...
	if (*((uint16*)(skb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021Q)) {
        #ifdef CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT
        if(!((0==(data[0]&0x01))||(skb->data[0]==0xff && skb->data[1]==0xff && 
        skb->data[2]==0xff&& skb->data[3]==0xff&& skb->data[4]==0xff&& 
        skb->data[5]==0xff)))
            goto Reserve_mctag;
        #endif
		#if defined(CONFIG_RTL_HW_VLAN_SUPPORT)
...
Reserve_mctag:
	...
	if (0==(skb->data[0]&0x01))		/*	unicast pkt only	*/
	{
	#ifdef CONFIG_RTL_VLAN_8021Q
		if(linux_vlan_enable)
		{
			pid=info->srcpid;
			if(pid<CACHE_ETH_DEV_MAX){				
				if(cached_eth_info[pid].flag&CACHE_TIME_OUT){
					memcpy(cached_eth_info[pid].mac, &skb->data[ETH_ALEN], ETH_ALEN);	
					cached_eth_info[pid].dev= cp_this->dev;	
					vid = *((unsigned short *)(skb->data+(ETH_ALEN<<1)+2));
					cached_eth_info[pid].vid= vid&0xfff;
					cached_eth_info[pid].flag &=~CACHE_TIME_OUT;
				}
			#ifdef CONFIG_RTL_PREV_L2_AGING
				memcpy(shortcut_sta_addr, &skb->data[0], ETH_ALEN);
			#endif
			...
			/*get dev when mac and vid are both match*/
			if((cached_wlan_dev = get_shortcut_dev(skb->data)) != NULL)
			{
				skb->dev = cached_wlan_dev;				
#if defined(CONFIG_COMPAT_NET_DEV_OPS)
				cached_wlan_dev->hard_start_xmit(skb, cached_wlan_dev);
#else
				cached_wlan_dev->netdev_ops->ndo_start_xmit(skb,cached_wlan_dev);
	#endif
				return;
			}	
		}
...
#ifdef CONFIG_ACT_PERF2
	int budget = 3;
	//unsigned long time_limit = jiffies + 1;
#endif
...
#ifdef CONFIG_RTL_PREV_L2_AGING
	if (++upd_l2_age_counter > 30)
	{
		extern int32 rtl819x_update_fdb_age(ether_addr_t *mac);
#ifdef CONFIG_RTL_VLAN_8021Q
		if(linux_vlan_enable){
			for(j=0;j<CACHE_ETH_DEV_MAX;j++){
				if(cached_eth_info[j].vid && 
					((*(uint32 *)cached_eth_info[j].mac) != 0))
					rtl819x_update_fdb_age((ether_addr_t *)cached_eth_info[j].mac);
			}
		}
		else
#endif
		{
			if ((*(uint32 *)cached_eth_addr) != 0) 
				rtl819x_update_fdb_age((ether_addr_t *)cached_eth_addr);
			#ifdef BR_SHORTCUT_C2
			if ((*(uint32 *)cached_eth_addr2) != 0) 
				rtl819x_update_fdb_age((ether_addr_t *)cached_eth_addr2);
			#endif
		}
		if ((*(uint32 *)shortcut_sta_addr) != 0) 
			rtl819x_update_fdb_age((ether_addr_t *)shortcut_sta_addr);
		upd_l2_age_counter = 0;
	}
#endif

#ifdef CONFIG_RTL_VLAN_8021Q 	
	if(cache_dev_age_counter == 0)
	{	
		for(j=0;j<CACHE_ETH_DEV_MAX;j++){
			cached_eth_info[j].flag |= CACHE_TIME_OUT;
		}		
	}
	cache_dev_age_counter++;
	cache_dev_age_counter=cache_dev_age_counter%300;
#endif
...
#ifdef CONFIG_AEI_MOCA_ENABLE
#if defined(CONFIG_RTK_VLAN_SUPPORT)
	if(memcmp((dev->name), MOCA_IFNAME, strlen(MOCA_IFNAME)) == 0)
	{
		moca_memory_open(dev);
	}
#else
	moca_memory_open(dev);
#endif
...
int act_gmac_do_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
#ifdef CONFIG_AEI_MOCA_ENABLE
	int ret;
#if defined(CONFIG_RTK_VLAN_SUPPORT)
	...
	{
#endif
		ret = moca_enet_ioctl(dev, rq, cmd);
		if(ret != -ENOIOCTLCMD)
		{
			return ret;
		}
	...
	return re865x_ioctl(dev,rq,cmd);
#else
	return (re865x_ioctl(dev,rq,cmd));
#endif
}
...
static const struct net_device_ops rtl819x_netdev_ops = {
	...
	.ndo_do_ioctl		= act_gmac_do_ioctl,
...
/*2007-12-19*/
	...
	/* reset MOCA chipset */
#if defined(CONFIG_AEI_HW_WECB3000) || defined(CONFIG_AEI_HW_JNR3000)
	RTL_W32(PIN_MUX_SEL2, RTL_R32(PIN_MUX_SEL2) | 0x3 << 13); // set LED_P0 as GPIO mode
#elif defined(CONFIG_AEI_HW_ECB3000)
	RTL_W32(PIN_MUX_SEL, RTL_R32(PIN_MUX_SEL) | 0x3 << 1); // set gpioh as GPIO mode
#endif
	rtl_gpio_out_init(MOCA_RESET_GPIO);
	rtl_gpio_out_val(MOCA_RESET_GPIO, 0);
	msleep(1000);
	rtl_gpio_out_val(MOCA_RESET_GPIO, 1);
	printk("Reset EN2510 Chip...OK\n");
#endif
...
//	rtlglue_printf("	creating eth0~eth%d...\n",totalVlans-1 );
	...
	for(i=0;i<totalVlans;i++)
	{
		...
		dev->do_ioctl = act_gmac_do_ioctl;
...
#ifdef CONFIG_AEI_MOCA_ENABLE
#if defined(CONFIG_RTK_VLAN_SUPPORT)
				if(memcmp((vlanconfig[i].ifname), MOCA_IFNAME, strlen(MOCA_IFNAME)) == 0)
				{
					memcpy((char*)dev->name, (char*)(&(vlanconfig[i].ifname)), strlen(MOCA_IFNAME) + 1);
				}
#endif
#endif
		...
		{
#ifdef CONFIG_AEI_MOCA_ENABLE
			/* make it support ethtool for MOCA interface */
#if defined(CONFIG_RTK_VLAN_SUPPORT)
			if(memcmp((vlanconfig[i].ifname), MOCA_IFNAME, strlen(MOCA_IFNAME)) == 0)
			{
#endif
				if(moca_memory_open(dev))
				{
					printk("moca_memory_open error\n");
				}
				SET_ETHTOOL_OPS(dev, &moca_ethtool_ops);
#if defined(CONFIG_RTK_VLAN_SUPPORT)
			}
#endif
#endif
	...
	((struct dev_priv*)((_rtl86xx_dev.dev[0])->priv))->dev_prev = NULL;
#endif
...
static int rtl_config_perport_perdev_vlanconfig(int mode)
{
	...
	#if defined(CONFIG_8198_PORT5_GMII) || defined(CONFIG_AEI_MOCA_ENABLE)
...
#if !defined(CONFIG_AEI_HW_WECB3000) && !defined(CONFIG_AEI_HW_ECB3000) && !defined(CONFIG_AEI_HW_JNR3000)
		vlanconfig[1].vid = 0;
