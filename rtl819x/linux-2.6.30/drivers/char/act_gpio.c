...
static int hw_button_netlink(int irq)
{
	int i = 0;
	...
	{
#ifdef DEBUG	
		printk("irq=%d, hw_buttons[i].gpio=%d\n",   irq ,  hw_buttons[i].gpio);
#endif
		if (irq == hw_buttons[i].gpio)
		{
			return msg_netlink_broadcast(&hw_buttons[i].message, hw_buttons[i].message_type, sizeof(hw_buttons[i].message));
		}
	}
...
static void hw_button_tasklet_func(unsigned long irq)
{
	hw_button_netlink(*(int *)irq);
}

DECLARE_TASKLET(hw_button_tasklet, hw_button_tasklet_func, (unsigned long)&irq_number);
...
static irqreturn_t hw_button_isr(int irq, void *dev_id)
{
	...
	btn->name, is_pressed ? "pressed" : "released",
