          ...
          const unsigned char *addr, const u32 *ip)
{
    ...
    if(head)
    {
        ...
        if (entry) {
            entry->need_info++; 
            //need_info: in case, fdb is created. but mynetwork doesn't recieve the first message.
            if(entry->ip != *ip || 3 == entry->need_info)
            {
                memcpy(&entry->ip, ip, 4);
                printk(KERN_DEBUG"%s update %p \n", __func__, entry);
...
void br_fdb_cleanup(unsigned long _data)
{
	...
	for (i = 0; i < BR_HASH_SIZE; i++) {
		...
		hlist_for_each_entry_safe(f, h, n, &br->hash[i], hlist) {
            ...
            #ifdef CONFIG_BR_IP_MAC_MAPPING
            f->live_ticks -= 1;
            if (time_before_eq(this_timer, jiffies) || f->live_ticks <=0) 
            #else
