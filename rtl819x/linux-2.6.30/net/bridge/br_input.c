...
ap_hcm_out:	
	...
	if (is_multicast_ether_addr(dest) && igmpsnoopenabled) {
		...
		if ( !(br->dev->flags & IFF_PROMISC) 
         ...
         && (((eth_hdr(skb)->h_proto == ETH_P_8021Q)&&ntohs(*((uint16*)(skb->data+2)))==ETH_P_IP) || (eth_hdr(skb)->h_proto == ETH_P_IP)))
         #else
		&& (eth_hdr(skb)->h_proto == ETH_P_IP))
		#endif
		{
            #ifdef CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT
            //panic_printk("%s %d srcVlanId:0x%x, srcPort 0x%x type 0x%x \n", __FUNCTION__, __LINE__, srcVlanId, srcPort, ntohs(*((uint16*)((unsigned char *)skb->data+2))));
            if (eth_hdr(skb)->h_proto == ETH_P_8021Q)
            {
                iph=(struct iphdr *)(skb_network_header(skb) + VLAN_HLEN);
                //panic_printk("%s %d iph->protocol %d \n", __FUNCTION__, __LINE__, iph->protocol);
            }
            else
			    iph=(struct iphdr *)skb_network_header(skb);
            #else
			...
			&& IPV6_MULTICAST_MAC(dest)		 
			#ifdef CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT
            && (((eth_hdr(skb)->h_proto == ETH_P_8021Q)&&ntohs(*((uint16*)(skb->data+2)))==ETH_P_IPV6) || (eth_hdr(skb)->h_proto == ETH_P_IPV6)))
            #else
			&& (eth_hdr(skb)->h_proto == ETH_P_IPV6))
			#endif
		{
			...
			if(mldSnoopEnabled)
			{
                ...
                if (eth_hdr(skb)->h_proto == ETH_P_8021Q)
                {
                    ...
                    //panic_printk("%s %d ipv6h->nexthdr %d \n", __FUNCTION__, __LINE__, ipv6h->nexthdr);
                }
                else
...
struct sk_buff *br_handle_frame(struct net_bridge_port *p, struct sk_buff *skb)
{
    ...
    if( __constant_htons(ETH_P_ARP) == skb->protocol)
    {
        br_fdb_update_ip_addr(p->br, skb); /*32287*/
    }
